...
def train_double_barreled_network(x_inputs, y_inputs, num_epochs):
    ...
    cost = functions.minus_corr(target_u, v)
    cost = cost.mean()
    ...
    updates = lasagne.updates.nesterov_momentum(cost, x_params, learning_rate=0.1)

    # train_fn = theano.function([x_input, y_input, u, v], cost, updates=updates)

    train_fn = theano.function([u_eval, y_input], cost, updates=updates)
    ...
    for epoch in range(num_epochs):
        ...
        train_err += train_fn(u_eval, y_inputs)
        train_batches += 1
